...
```{r graph_USmap_2022_DSCI_byCounty, include = TRUE, message = FALSE}
counties %>% 
  left_join(drought_fips_yearlyAverage_county, by = c("NAME" = "state_name")) %>% 
  filter(Year == 2022) %>% 
  ggplot(aes(fill = Avg_DSCI)) + 
  labs(title = "2022 Average Drought Severity and Coverage Index (DSCI) by US State County") +
  geom_sf() +
  scale_fill_continuous(type = "viridis")
```
...
animate(USmap_timelapse_DSCI_byState, renderer = gifski_renderer(), duration = 20, start_pause = 2, end_pause = 2)
```

```{r graph_USmap_timelapse_DSCI_byCounty, include = TRUE, message = FALSE, warning = FALSE}
#create a US map and plot average DSCI 2000-2022
USmap_timelapse_DSCI_byCounty <- states %>% 
  left_join(drought_fips_yearlyAverage_county, by = c("NAME" = "state_name")) %>% 
  ggplot(aes(fill = Avg_DSCI)) + 
 # labs(title = "2022 Average Drought Severity and Coverage Index (DSCI) by US State") +
  geom_sf() +
  ...
  labs(title = "Average Drought Severity and Coverage Index (DSCI) by US County",
       subtitle = "Year: {closest_state}")

animate(USmap_timelapse_DSCI_byCounty, renderer = gifski_renderer(), duration = 20)
```

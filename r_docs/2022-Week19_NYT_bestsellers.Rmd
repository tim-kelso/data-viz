...
## Graphs  
```{r graph1_title_length, include = TRUE, message = FALSE}
nyt_titles_orig %>% 
...
```{r graph2_average_title_length_by_year, include = TRUE, message = FALSE}
nyt_titles_orig %>% 
  mutate(title_length = str_length(title)) %>% 
  group_by(year) %>% 
...
```{r graph3_title_length_by_total_weeks, include = TRUE, message = FALSE}
nyt_titles_orig %>% 
  mutate(title_length = str_length(title)) %>% 
...
```
```{r graph4_bestselling_authors_by_total_weeks, include = TRUE, message = FALSE}
nyt_titles_orig %>% 
  group_by(author) %>% 
  ...
  theme_void()
```
```{r graph5_most_common_words, include = TRUE, message = FALSE, warning = FALSE}
...
nyt_titles_orig %>%
  ...
  geom_text_wordcloud(seed = 3590) + 
  labs(title = "Most Common Words in Bestsellers Titles")+
  scale_color_viridis_c(option = "viridis") +
  theme_void()
```
